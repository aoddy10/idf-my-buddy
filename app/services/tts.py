...
class TTSService(LoggerMixin):
    ...
    def _setup_speechbrain(self):
        ...
        try:
            ...
            self._speechbrain_tacotron = Tacotron2.from_hparams(
                ...
                savedir=settings.MODEL_CACHE_DIR / "tacotron2",
            ...
            self._speechbrain_vocoder = HIFIGAN.from_hparams(
                ...
                savedir=settings.MODEL_CACHE_DIR / "hifigan",
                run_opts={"device": self._device}
            )
    ...
    def _choose_best_engine(self, language: str, voice: Optional[str]) -> str:
    ...
    def _map_voice_to_openai(self, voice: Optional[str]) -> str:
        ...
        return voice_mapping.get(voice, "nova")  # Default to nova
    ...
    def _pyttsx3_synthesis(self, text: str, voice: Optional[str], speed: float) -> Dict[str, Any]:
    ...
    async def get_available_voices(self, engine: Optional[str] = None) -> Dict[str, List[str]]:
        """Get available voices for each engine."""
        
        voices = {}
        
        if engine is None or engine == "openai":
            if self._openai_client:
                voices["openai"] = ["alloy", "echo", "fable", "onyx", "nova", "shimmer"]
        
        if engine is None or engine == "pyttsx3":
