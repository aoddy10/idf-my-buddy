...
from app.schemas.common import (
    BaseResponse, LanguageCode, FileUpload, ProcessedMedia,
...
class SpeechRecognitionRequest(BaseModel):
    ...
    language_hint: Optional[LanguageCode] = Field(None, description="Expected language hint")
    model_preference: Optional[str] = Field(None, description="ASR model preference")
    ...
    max_speakers: Optional[int] = Field(None, ge=2, le=10, description="Maximum number of speakers")
    ...
    domain_hint: Optional[str] = Field(None, description="Domain hint (medical, legal, etc.)")
...
class SpeechSegment(BaseModel):
    ...
    speaker_id: Optional[int] = Field(None, description="Speaker identifier (if diarization enabled)")
...
class SpeechRecognitionResponse(BaseResponse):
    ...
    noise_level: Optional[str] = Field(None, description="Background noise level")
    speech_rate_wpm: Optional[int] = Field(None, description="Speech rate (words per minute)")
...
class VoiceConversationRequest(BaseModel):
    ...
    conversation_id: Optional[UUID] = Field(None, description="Conversation context ID")
    
    # Processing preferences
    source_language: Optional[LanguageCode] = Field(None, description="Input language (auto-detect if None)")
    ...
    response_language: Optional[LanguageCode] = Field(None, description="Voice response language")
    
    # Context
    location: Optional[Dict[str, float]] = Field(None, description="User location context")
...
class VoiceConversationResponse(BaseResponse):
    ...
    translated_response: Optional[str] = Field(None, description="Translated response")
    
    # Voice output (if requested)
    voice_response_url: Optional[str] = Field(None, description="Voice response audio URL")
    voice_duration_seconds: Optional[float] = Field(None, description="Voice response duration")
    ...
    tts_time_ms: Optional[int] = Field(None, description="TTS processing time")
...
class AudioTranslationResponse(BaseResponse):
    ...
    translated_audio_url: Optional[str] = Field(None, description="Translated audio URL")
    translated_duration_seconds: Optional[float] = Field(None, description="Translated audio duration")
...
class VoiceCloneRequest(BaseModel):
    ...
    training_duration: Optional[int] = Field(None, description="Minimum training duration")
...
class AudioAnalysisResponse(BaseResponse):
    ...
    bit_rate: Optional[int] = Field(None, description="Bit rate")
    ...
    estimated_words: Optional[int] = Field(None, description="Estimated word count")
    speaking_rate_wpm: Optional[int] = Field(None, description="Speaking rate")
    
    # Emotional analysis (if enabled)
    emotional_tone: Optional[str] = Field(None, description="Detected emotional tone")
    emotion_confidence: Optional[float] = Field(None, description="Emotion detection confidence")
    
    # Speaker analysis (if enabled)
    speaker_count: Optional[int] = Field(None, description="Number of speakers detected")
    primary_speaker_gender: Optional[VoiceGender] = Field(None, description="Primary speaker gender")
